...
func (r *producer) Send(data string) bool {

	// 获取一个频道
	ch, err := r.connect.Channel()
	r.occurError = errorDeal(err)
	...
	_, err = ch.QueueDeclare(
		r.queueName, // 队列名称
		r.durable,   //是否持久化，false模式数据全部处于内存，true会保存在erlang自带数据库，但是影响速度
		!r.durable,  //生产者、消费者全部断开时是否删除队列。一般来说，数据需要持久化，就不删除；非持久化，就删除
	...
	err = ch.Publish(
		...
		r.queueName, // 注意：简单模式 key 表示队列名称
		false,
		false,
		amqp.Publishing{
	...
	if r.occurError != nil { //  发生错误，返回 false
		return false
	} else {
		return true
	}
