...
* 新增    
    ...
    3.`gorm v2` 接入层 `utils` 增加 `Create` 函数的参数类型非指针时拦截检查逻辑避免发生 `panic`.
    4.`gorm v2` 接入层 `utils` 增加 `Save、Update` 函数的参数类型非指针时拦截检查逻辑,以便支持 `gorm` 的所有回调函数.  
    5.为了完美支持第4条功能，今后在使用 `gorm 的函数 `Create 、Save、Update ` 时请统一传递指针类型的参数.  
* 更新
    1.验证码控制器文件单词拼写错误修正.  
    2.路由中的一些注释更新. 
    3.所有依赖包更新至最新版，与包相关的接入层(utils)相关部分也同步更新.  
