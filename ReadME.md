...
>   1.编写更多的单元测试。   
>   2.增加`Aop`面向切面编程功能。     
>   3.本项目骨架在新项目的应用中，将基础功能继续扩充，修复可能的bug。       

V 1.0.19   2020-06-05   
>   1.增加函数级别的`发布、订阅`功能，该模式主要将与主业务有弱关联关系的一组(如果只是一个子业务，就没有必要用这种办法)子业务进行了简单解耦。    
>   2.用法 [详情](Test/ObserverMode_test.go)  
>   3.增加 `nginx` 与 `supervisor` 部署相关的文档。  
