...
#### 版本
V 1.0.10   2020-04-28(dev分支功能预告)   
>   1.`websocket`功能开发进行中（计划2020.5.1放假之前开发完成，合并到master）,特色如下：  
>   1.屏蔽底层繁琐的基础设置，使用超级简单。      
>   2.严格按照`websocket`协议实现，服务器、浏览器自动隐式维护心跳，开发者只需要关注业务的核心数据交互，无需额外维护任何形式的心跳数据包。  
