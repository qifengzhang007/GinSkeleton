...
func (u *UsersModel) OauthRefreshToken(userId, expiresAt int64, oldToken, newToken, clientIp string) bool {
	...
	if u.Exec(sql, newToken, time.Unix(expiresAt, 0).Format(variable.DateFormat), clientIp, userId, oldToken).Error == nil {
		sql = "UPDATE  web.tb_users   SET  login_times=isnull(login_times,0)+1,last_login_ip=?,last_login_time=?  WHERE   id=?  "
...
// 禁用一个用户的: 1.tb_users表的 status 设置为 0，web.tb_oauth_access_tokens 表的所有token删除
...
func (u *UsersModel) ShowOneItem(userId float64) (*UsersModel, error) {
	sql := "SELECT id, user_name,pass, real_name, phone, status FROM  web.tb_users  WHERE status=1 and   id=? limit 1"
...
func (u *UsersModel) Show(userName string, limitStart, limitItems int) (counts int64, temp []UsersModel) {
	if counts = u.counts(userName); counts > 0 {
		sql := `
		SELECT  id, user_name, real_name, phone, status, TO_CHAR(created_at,'YYYY-MM-DD HH24:MI:SS') as created_at, TO_CHAR(updated_at,'YYYY-MM-DD HH24:MI:SS') as updated_at
