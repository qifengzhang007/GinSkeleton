...
func CreateBaseSqlFactory(sql_type string) (res *BaseModel) {
	...
	case "mysql":
		if mysql_driver == nil {
			mysql_driver = sql_factory.Init_sql_driver(sql_type)
		}
		// Ping() 命令表示检测数据库连接是否ok，必要时从连接池建立一个连接
		if err := mysql_driver.Ping(); err != nil {
			// 重试
			mysql_driver = sql_factory.GetOneEffectivePing(sql_type)
			// 如果重试成功
			if err := mysql_driver.Ping(); err == nil {
	...
	case "sqlserver", "mssql":
		if sqlserver_driver == nil {
			sqlserver_driver = sql_factory.Init_sql_driver(sql_type)
		}
		// Ping() 命令表示检测数据库连接是否ok，必要时从连接池建立一个连接
		if err := sqlserver_driver.Ping(); err != nil {
			// 重试
			sqlserver_driver = sql_factory.GetOneEffectivePing(sql_type)
			// 如果重试成功
			if err := sqlserver_driver.Ping(); err == nil {
	...
	default:
		variable.Zap_Log.Error(my_errors.Errors_Db_Driver_NotExists)
	}
	...
	if stm, err := b.db_driver.Prepare(sql); err == nil {
		if res, err := stm.Exec(args...); err == nil {
			if affectNum, err := res.RowsAffected(); err == nil {
				...
				variable.Zap_Log.Error(my_errors.Errors_Db_Execute_RunFail, zap.Error(err))
			}
		} else {
			variable.Zap_Log.Error(my_errors.Errors_Db_Prepare_RunFail, zap.Error(err))
		}
	}
	return -1
...
func (b *BaseModel) QuerySql(sql string, args ...interface{}) *sql.Rows {
	if stm, err := b.db_driver.Prepare(sql); err == nil {
		...
		if Rows, err := stm.Query(args...); err == nil {
			...
			variable.Zap_Log.Error(my_errors.Errors_Db_Query_RunFail, zap.Error(err))
		}
	} else {
		variable.Zap_Log.Error(my_errors.Errors_Db_Prepare_RunFail, zap.Error(err))
	}
...
func (b *BaseModel) QueryRow(sql string, args ...interface{}) *sql.Row {
	if stm, err := b.db_driver.Prepare(sql); err == nil {
		...
		variable.Zap_Log.Error(my_errors.Errors_Db_QueryRow_RunFail, zap.Error(err))
		return nil
	}
}
...
func (b *BaseModel) ExecuteSqlForMultiple(args ...interface{}) int64 {
	if res, err := b.stm.Exec(args...); err == nil {
		...
		variable.Zap_Log.Error(my_errors.Errors_Db_ExecuteForMultiple_Fail, zap.Error(err))
	}
	return -1
}
...
func (b *BaseModel) BeginTx() *sql.Tx {
	if tx, err := b.db_driver.Begin(); err == nil {
		...
		variable.Zap_Log.Error(my_errors.Errors_Db_Transaction_Begin_Fail + err.Error())
	}
	return nil
}
